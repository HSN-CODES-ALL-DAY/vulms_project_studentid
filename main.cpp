...
int main(int argc,const char* argv[]){
  ...
  if (iname!=istudent){
    std::cout << "please input both id and name use -h for help\n";
    return 3;
  }
  //small error checking 
  if (stringlength(student)<4){
      std::cout << "student id cannot be less than 4 characters\n";
      return 1;
  }
  //
  studentid hsn (student,name);
  if ( hsn.errorstate()){
    std::cout<<"studentid cannot have more than 3 alphabets\n";
    return 2;
  }
